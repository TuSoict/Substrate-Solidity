...
contract NftMarket is ERC721URIStorage, Ownable{
    ...
    uint public listingPrice = 0.02 ether;
    ...
    mapping (uint => uint) private _idToIndex;
    mapping(address => mapping(uint => uint)) private _ownerNftItem;
    ...
    uint[] private _ownerNfts;


    using Counters for Counters.Counter;

    constructor() ERC721("VBINFT","VBIN")   {}
    ...
    function tokenByIndex(uint tokenIndex) public view returns(uint){
        return _allNfts[tokenIndex];
    }
    ...
    function mintToken(string memory tokenURI, uint price) public payable returns(uint256){
        require(!tokenURIexits(tokenURI),"Token URI is exits");
        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        _setTokenURI(newItemId,tokenURI);
        createNftItem(newItemId, price);
    ...
    function buyNft(uint tokenId) public payable {
        ...
        require(owner != msg.sender,"Owner must not be Sender");
    ...
    function _beforeTokenTransfer(address from,address to,uint tokenId) internal virtual override {
        ...
        addNftItemToArray(tokenId);
    }
    function addNftItemToArray(uint tokenId) private {
        _idToIndex[tokenId] = _allNfts.length;
    ...
    function _removeTokenFromOwner(address from,uint tokenId) private{
    ...
    function _removeTokenFromAllTokens(uint tokenId) private {
    ...
    function _addTokenToAllTokens(uint tokenId) private {
